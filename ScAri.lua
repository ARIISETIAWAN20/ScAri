...
            return HttpService:JSONDecode(readfile("ARI HUB/ARI HUB.json"))
        end)
...
        makefolder("ARI HUB")
    end
    writefile("ARI HUB/ARI HUB.json", HttpService:JSONEncode(settings))
end
...
    
    espParts[targetPlayer] = {highlight = highlight, billboard = billboard}
end

local function updateESP()
    for p, esp in pairs(espParts) do
        if p and p.Character and rootPart then
            local char = p.Character
            local hrp = char:FindFirstChild("HumanoidRootPart")
            if hrp then
                local dist = (hrp.Position - rootPart.Position).Magnitude
                if dist <= settings.espDistance then
                    if not esp.highlight.Parent then esp.highlight.Parent = char end
                    if not esp.billboard.Parent then esp.billboard.Parent = char end
                    local distLabel = esp.billboard:FindFirstChild("DistanceLabel")
                    if distLabel then
...
    if clipConnection then clipConnection:Disconnect() end
...
        if not settings.antiClipEnabled then
            -- matikan clip
            if character and rootPart then
...
        -- Pastikan CanCollide tetap false
        if character and rootPart then
